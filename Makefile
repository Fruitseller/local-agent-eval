...
test: check baseline
	$(PYTHON) -c 'import json, pathlib; p=pathlib.Path("results/$(BASELINE_ID)"); s=json.loads((p/"summary.json").read_text()); assert s["totals"]["tasks"] == 5; results=[json.loads(f.read_text()) for f in p.glob("*/result.json")]; assert len(results) == 5; suites=[x for r in results for x in r["suites"]]; assert suites and all(x["exit_code"] is not None and not x["timed_out"] for x in suites); print(f"validated {len(results)} tasks and {len(suites)} runnable public suites")'
